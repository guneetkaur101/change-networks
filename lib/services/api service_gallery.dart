...
class ApiService {
  ...
  Future<void> login({required String email, required String password}) async {
    final r = await _dio.post('/admin/login', data: {'email': email, 'password': password});
    ...
    // If API returns a token, attach for Bearer auth (cookies also set by CookieManager).
  ...
  Future<GalleryResponse> _fetchCategory(
    ...
    try {
      ...
      // If the endpoint isn’t present in this env, serve empty instead of crashing UI
  ...
  // Allows custom query param keys (needed for ?limit=&search=)
  Future<GalleryResponse> _fetchCategoryWith(
    ...
    try {
      ...
      final metaMap = (b['meta'] as Map<String, dynamic>?) ?? const {};
      final meta = PaginationMeta(
        currentPage: (metaMap['currentPage'] ?? fallbackPage) as int,
        totalPages: (metaMap['totalPages'] ?? 1) as int,
        totalItems: (metaMap['totalItems'] ?? items.length) as int,
        itemsPerPage: (metaMap['itemsPerPage'] ?? fallbackLimit) as int,
      );
      return GalleryResponse(items: items, meta: meta, message: (b['message'] ?? '').toString());
    } on DioException catch (e) {
      if (e.response?.statusCode == 404) {
        // Search returning 404 -> interpret as "no matches"
  ...
  /// Supports both:
  /// - /gallery/switches?limits=12&page=1      (no search)
  /// - /gallery/switches?limit=4&page=1&search=... (with search)
  Future<GalleryResponse> getSwitches({int limits = 10, int page = 1, String? search}) {
    if (search == null || search.trim().isEmpty) {
      return _fetchCategory('/gallery/switches', limits: limits, page: page);
    }
    return _fetchCategoryWith(
      '/gallery/switches',
      query: {'limit': limits, 'page': page, 'search': search.trim()},
      fallbackPage: page,
      fallbackLimit: limits,
    );
    // NOTE: server uses "limit" (singular) when search is present.
  }
  ...
  /// Categories (product codes grouped by category)
  Future<GalleryCategories> getCategories({int limits = 10}) async {
    final r = await _dio.get('/gallery/cateogries', queryParameters: {'limits': limits}); // API spelling
    final b = (r.data is Map<String, dynamic>) ? r.data as Map<String, dynamic> : {};
    if (b['success'] != true) {
      throw DioException(
        requestOptions: r.requestOptions,
